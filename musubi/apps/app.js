		...
		for (i=0; i<users.length; i++) //looping through all users
		{
			...
			if (temp_progress.length == 3) //if done
			{
				...
				if (screen_type == "truth")
				{
					...
					truth_content += ("<li><h3>" + name+ "</h3><p><strong>Truth: "+text+"</strong></p><p>"+answer+"</p><img height='110px' width='85px' src='http://www.myctb.org/wst/npaoeval/Picture%20Library/Checkmark.png'/></li>");
					totalTruths++;
				}
				else
				{
					dare_content += ("<li class='link' user_name='"+name+"'><h3>" + name + "</h3><p><strong>Dare: "+text+"</strong></p><p>"+"Click to see proof"+"</p><img height='90px' width='75px' src='http://www.wpclipart.com/signs_symbol/checkmarks/checkmark_in_circle/ok_checkmark_red.png'/></li>");
					totalDares++;
				}
			}
		}
